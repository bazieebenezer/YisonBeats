...
export default function AdminDashboard() {
  const stats = [
    { name: "Chiffre d'affaires", value: "245 000 FCFA", change: "+12%", icon: DollarSign, color: "text-green-500" },
    { name: "Commandes", value: "18", change: "+5%", icon: ShoppingBag, color: "text-blue-500" },
    { name: "Téléchargements", value: "142", change: "+24%", icon: Download, color: "text-purple-500" },
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <Card key={stat.name} className="border-none bg-white overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center bg-slate-50", stat.color)}>
                  <stat.icon className="h-5 w-5" />
                </div>
                <div className="flex items-center gap-1 text-xs font-bold text-green-500 bg-green-50 px-2 py-0.5 rounded-full">
                    ...
                    <tr key={order.id} className="hover:bg-slate-50 transition-colors text-sm">
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          order.status === "Payé" ? "bg-green-100 text-green-600" : "bg-yellow-100 text-yellow-600"
                        )}>
                          {order.status}
                        </span>
                      </td>
