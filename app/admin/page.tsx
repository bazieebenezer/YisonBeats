          ...
          <Card key={stat.name} className="border-none bg-card  overflow-hidden">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1 text-xs font-bold text-green-500 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-0.5 rounded-lg">
                  {stat.change} <ArrowUpRight className="h-3 w-3" aria-hidden="true" />
                </div>
              </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border/60">
                  {[
                    ...
                    <tr key={order.id} className="hover:bg-muted/50 transition-colors text-sm">
                      <td className="px-6 py-4 font-bold text-primary tabular">{order.id}</td>
                      <td className="px-6 py-4 font-medium">{order.customer}</td>
                      <td className="px-6 py-4 text-muted-foreground">{order.items}</td>
                      <td className="px-6 py-4 font-extrabold tabular">{order.amount}</td>
            ...
            {[
              ...
              <div key={i} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="h-8 w-8 rounded-lg bg-muted flex items-center justify-center text-xs font-bold text-muted-foreground tabular">
                    {i + 1}
                  </div>
                  <div>
                    <p className="text-sm font-bold">{p.name}</p>
                    <p className="text-[10px] text-muted-foreground tabular">{p.sales} ventes</p>
                  </div>
                </div>
                <p className="text-sm font-extrabold tabular">{p.revenue}</p>
              </div>
            ))}
          </CardContent>
        </Card>
